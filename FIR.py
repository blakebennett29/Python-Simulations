...
def kaiser_beta_for_enob(bits):
    ...
    A_target ~ SNR(bits) = 6.02*bits + 1.76 dB
    """
...
# ============================
# 2) Build EVEN-WIDTH bands
# ============================

def make_even_bands(f_min, f_max, num_bands, trans_frac=0.3):
    """
    Make evenly spaced bands between f_min and f_max.
...
def design_bandpass_fir(num_taps, fs, fpl, fpu, beta):
    """Design a bandpass FIR with passband [fpl, fpu] (Hz) using a Kaiser window."""
    w1 = fpl / (fs / 2.0)
    w2 = fpu / (fs / 2.0)
    return firwin(
        num_taps,
...
# ============================
# 4) Design all bands & extract specs
# ============================
...
H_all = np.array(H_all)          # shape (num_bands, num_points)
mag_all_db = np.array(mag_all_db)

# ============================
# 5) Combined response & overlap (UNSCALED)
# ============================
...
# ============================
# 7) Print auto-extracted specs + ENOB
# ============================

print("\n=== Auto Extracted Specs per Even-width Band (Kaiser) ===")
enob_list = []
...
# ============================
# 8) Generate .coe files (NO ALPHA SCALING)
# ============================

all_coeffs_int = []

for h, b in zip(coeffs_list, bands):
    q = quantize_coeffs(h, coeff_bits)  # unscaled h
    all_coeffs_int.append(q)
    fname = f"{b['name']}_fir_{coeff_bits}b_kaiser.coe"
    write_coe(fname, q, radix=10)

write_all_coeffs_coe("all_bands_combined_kaiser.coe", all_coeffs_int, radix=10)
...
plt.title("Even-width multi-band FIR (Kaiser): bands, sum, and noise floors")
plt.ylim([-160, 10])
...
plt.title("How many bands are 'strong' at each frequency (even-width, Kaiser)")
plt.grid(True, which="both", ls=":")
plt.tight_layout()

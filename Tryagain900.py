...
def worst_stopband_level(freqs, H, fsl, fsu):
    """Worst-case stopband level (dB) outside [fsl, fsu]."""
    mag_db = 20.0 * np.log10(np.maximum(np.abs(H), eps))
    stop_mask = (freqs <= fsl) | (freqs >= fsu)
    return float(np.max(mag_db[stop_mask]))
...
# --- Adjust Band0 so its peak is similar to Bands 1–3 ---
coeffs_tapered_tmp = coeffs_list * gains[:, None]
H_tapered_tmp = []
for h in coeffs_tapered_tmp:
    _, H = compute_response(h, fs, num_points)
    H_tapered_tmp.append(H)
H_tapered_tmp = np.array(H_tapered_tmp)

band_peaks_db_tmp = []
for k, b in enumerate(bands):
    mask = (freqs >= b["fpl"]) & (freqs <= b["fpu"])
    mag_band = np.abs(H_tapered_tmp[k, mask])
    band_peaks_db_tmp.append(20.0 * np.log10(np.maximum(mag_band.max(), eps)))

# Use average of Bands 1–3 as Band0 target
target_peak0_db = float(np.mean(band_peaks_db_tmp[1:4]))
peak0_db = band_peaks_db_tmp[0]
delta0_db = target_peak0_db - peak0_db  # negative if Band0 is hotter

# Only cut; never boost
if delta0_db < 0:
    band0_manual_gain = 10.0**(delta0_db / 20.0)
else:
    band0_manual_gain = 1.0

gains[0] *= band0_manual_gain

print("\nPer-band taper gains (linear, after Band0 equalization):")
for i, g in enumerate(gains):
    print(f"  {bands[i]['name']}: {g:.6f}")

# Apply final per-band taper
...
for k, b in enumerate(bands):
    ...
    worst_sb = worst_stopband_level(freqs, H_final[k], b["fsl"], b["fsu"])
    worst_stop_db.append(worst_sb)

print("\n=== Band heights and EQ gains to get back to original 0 dB ===")
print("Band  centerHz  per-band-gain  net-atten-dB  gain-to-undo-dB  peak-dB  worst-stop-dB")
for i, (c, g, att, ud, pk, ws) in enumerate(
    zip(band_centers, gains, atten_db, undo_db, band_peaks_db, worst_stop_db)
):
    print(f"{i:2d}  {c:8.1f}    {g:8.5f}   {att:10.3f}   {ud:10.3f}   {pk:8.3f}   {ws:8.3f}")
